			...
			do
				...
				then
					...
					then
						...
						cp gd32f470.bin /tmp/$f/$i/$SUFFIX1
					else
						mkdir -p /tmp/$f/$i/$SUFFIX1/$SUFFIX2/
						cp gd32f470.bin /tmp/$f/$i/$SUFFIX1/$SUFFIX2/
					fi
				fi
...
find . -name gd32f470.bin | sort | xargs ls -al
find . -name gd32f470.bin | xargs ls -al | wc -l